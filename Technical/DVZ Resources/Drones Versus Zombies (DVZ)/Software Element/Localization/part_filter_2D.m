...
sigma_laser = 0.5;       %std dev of range measurement [m]

sigma_odom = 0.5;       % std dev of odometry data [m]

sigma_rand = 0.2;

num_particles = 500;     % number of particles being tracked

num_beams = 15;

t = linspace(0, 20, 40)'; % time [s]

x0 = 7;                 % x initial true position [m]
y0 = 13;                 % y initial true position [m]
psi0 = pi/2;                % psi initial true position [rad]
...
movement_x = 1*sin(0.01*t);    
% movement_y = zeros(length(t), 1);
movement_y =4*sin(0.5*t);
...
for n = 2:length(t)
    ...
    % Random probability
    part_prob = part_prob + sigma_rand;
    % Normalize to 1
