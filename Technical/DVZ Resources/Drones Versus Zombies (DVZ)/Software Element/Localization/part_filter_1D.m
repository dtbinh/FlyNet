...
for n = 2:length(t)
    ...
    part_array(:, n) = part_array(:, n-1) + movement(n);
    
    % Add odometry noise
    part_array(:, n) = part_array(:, n) + sigma_odom*randn(num_particles, 1);
...
% Error
subplot(2,1,2)
plot(t, abs(part_med - x_true).^2, '-r')
hold on
plot(t, abs(part_mean - x_true).^2, '-b')
hold off
xlabel('Time [s]')
ylabel('Abs Sqrd Err')
legend('Median', 'Mean');
