...
############################################################################################################
def velocity_controller(master,quad,target_velx_inertial,target_vely_inertial,target_ca_roll,target_ca_pitch,x,y,inertial_yaw,current_rc_channels,ca_active_flag):
	# Calculate delta t and set previous time at current time
	...
	if ca_active_flag == 0:
		...
		quad.target_ca_roll = target_ca_roll
		quad.target_ca_pitch = target_ca_pitch
		
		velx_pitch_pwm_send = angle2pwm(quad,quad.target_ca_pitch,'pitch')
		vely_roll_pwm_send = angle2pwm(quad,quad.target_ca_roll,'roll')
		
		# Filter and save pitch and roll pwm send values
		pitch_pwm_send = rc_filter(velx_pitch_pwm_send,quad.pitch_pwm_min,quad.pitch_pwm_max)
		roll_pwm_send = rc_filter(vely_roll_pwm_send,quad.roll_pwm_min,quad.roll_pwm_max)
		pitch_pwm_send = round(pitch_pwm_send)
		roll_pwm_send = round(roll_pwm_send)
...
def angle2pwm(quad,angle,controller):
	...
	if controller == 'roll':
		radio_trim = 1631#1597
		radio_max = 2051#2022
		radio_min = 1188#1164
		dead_zone = 30
	elif controller == 'pitch':
		radio_trim = 1413#1459
		radio_max = 1836#1877
		radio_min = 978#1107
		dead_zone = 30
	elif controller == 'throttle':
		radio_trim = 1107#1107
		radio_max = 1929#1929
		radio_min = 1107#1107
		dead_zone = 30
	elif controller == 'yaw':
		radio_trim = 1555#1558
		radio_max = 1983#1984
		radio_min = 1122#1125
		dead_zone = 40
