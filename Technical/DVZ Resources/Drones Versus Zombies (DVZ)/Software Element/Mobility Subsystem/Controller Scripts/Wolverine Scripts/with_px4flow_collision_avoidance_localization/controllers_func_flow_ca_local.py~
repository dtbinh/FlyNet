############################################################################################################
# controllers_func_flow_ca_local.py
...
def init_write_alt(filename):
	...
	f_alt.write("%Time, filtered_px4flow_alt, RC_P, RC_I, RC_D, rc_T_send, vicon_x, vicon_y, vicon_z, pixhawk_phi, pixhawk_theta, pixhawk_psi, target_alt, amcl_x, amcl_y, amcl_z, amcl_psi, unfiltered_px4flow_alt\n")
	f_alt.truncate()
	f_alt.close()
	return f_alt
############################################################################################################
...
############################################################################################################
def write_alt(filename,f_alt,quad,vicon_pos,pix_att,flow_alt,x_map,y_map,z_map,psi_map):
	f_alt = open(filename,'a+')
	f_alt.write("%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f\n" % (time.time(), quad.filtered_alt, quad.alt_RC_P, quad.alt_RC_I, quad.alt_RC_D, quad.T_send, vicon_pos[0], vicon_pos[1], vicon_pos[2], pix_att[0], pix_att[1], pix_att[2], quad.target_alt, x_map, y_map, z_map, psi_map, flow_alt))
	f_alt.close()
	return f_alt
############################################################################################################
...
def init_write_velocity(filename):
	...
	f_vel.write("%Time, velx_body, Px, Ix, Dx, pitch_angle_send, pitch_pwm_send, vely_body, Py, Iy, Dy, roll_angle_send, roll_pwm_send, vicon_x, vicon_y, vicon_z, pixhawk_phi, pixhawk_theta, pixhawk_psi, target_velx_inertial, target_vely_inertial, target_velx_body, target_vely_body, amcl_x, amcl_y, amcl_z, amcl_psi\n")
	f_vel.truncate()
	f_vel.close()
	return f_vel
############################################################################################################
...
def init_write_position(filename):
	...
	f_pos.write("%Time, velx_inertial_send, Px, Ix, Dx, vely_inertial_send, Py, Iy, Dy, vicon_x, vicon_y, vicon_z, pixhawk_phi, pixhawk_theta, pixhawk_psi, target_x, target_y, amcl_x, amcl_y, amcl_z, amcl_psi\n")
	f_pos.truncate()
	f_pos.close()
	return f_pos
############################################################################################################
...
############################################################################################################
def write_position(filename,f_pos,quad,vicon_pos,pix_att,x_map,y_map,z_map,psi_map):
	f_pos = open(filename,'a+')
	f_pos.write("%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f\n" % (time.time(), quad.velx_inertial_send, quad.posx_P, quad.posx_I, quad.posx_D, quad.vely_inertial_send, quad.posy_P, quad.posy_I, quad.posy_D, vicon_pos[0], vicon_pos[1], vicon_pos[2], pix_att[0], pix_att[1], pix_att[2], quad.target_x, quad.target_y, x_map, y_map, z_map, psi_map))
	f_pos.close()
	return f_pos
############################################################################################################
...
def init_write_rc(filename):
	...
	f_rc.write("%Time, rc_roll, rc_pitch, rc_throttle, rc_yaw, send_roll, send_pitch, send_throttle, send_yaw, vicon_x, vicon_y, vicon_z, pixhawk_phi, pixahwk_theta, pixhawk_psi\n")
	f_rc.truncate()
	f_rc.close()
	return f_rc
############################################################################################################
