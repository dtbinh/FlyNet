...
############################################################################################################
def alt_controller_vicon(master,quad,target_alt,alt,current_rc_channels):
	...
	#rc_T_send = (quad.base_rc_throttle + RC_P + RC_I + RC_D)/(math.cos(roll)*math.cos(pitch))
...
############################################################################################################
def init_write_alt_vicon(filename):
...
############################################################################################################
def write_alt_vicon(filename,f_alt,alt,quad,vicon_pos,vicon_orient,flow_alt):
	f_alt = open(filename,'a+')
	f_alt.write("%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f\n" % (time.time(), quad.filtered_alt, quad.alt_RC_P, quad.alt_RC_I, quad.alt_RC_D, quad.T_send, vicon_pos[0], vicon_pos[1], flow_alt, vicon_orient[0], vicon_orient[1], vicon_orient[2], quad.target_alt))
	f_alt.close()
	return f_alt
############################################################################################################

############################################################################################################
def init_write_flow_vicon(filename):
...
############################################################################################################
def write_flow_vicon(filename,f_flow,flow_alt,quality,quad,vicon_pos,vicon_orient):
	f_flow = open(filename,'a+')
...
############################################################################################################
def velocity_controller_vicon(master,quad,target_velx_inertial,target_vely_inertial,target_velx_body,target_vely_body,x,y,inertial_yaw,current_rc_channels,ca_active_flag):
	# Calculate delta t and set previous time at current time
...
############################################################################################################
def init_write_velocity_vicon(filename):
...
############################################################################################################
def write_velocity_vicon(filename,f_vel,quad,vicon_pos,pix_att,flow_alt):
	f_vel = open(filename,'a+')
...
############################################################################################################
def position_controller_vicon(master,quad,target_x,target_y,x,y):
	# Calculate delta t and set previous time at current time
...
############################################################################################################
def init_write_position_vicon(filename):
...
############################################################################################################
def write_position_vicon(filename,f_pos,quad,vicon_pos,pix_att,flow_alt):
	f_pos = open(filename,'a+')
...
############################################################################################################
def init_write_rc_vicon(filename):
...
############################################################################################################
def write_rc_vicon(filename,f_rc,quad,current_rc_channels,vicon_pos,pix_att,flow_alt):
	f_rc = open(filename,'a+')
