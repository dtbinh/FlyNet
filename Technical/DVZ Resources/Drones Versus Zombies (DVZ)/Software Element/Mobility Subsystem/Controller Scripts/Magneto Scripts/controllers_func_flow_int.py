...
############################################################################################################
def takeoff_controller(master,quad,target_alt,alt):
	# Calculate delta t and set previous time at current time
	delta_t = time.time() - quad.previous_time_takeoff
	if quad.previous_time_takeoff == 0:
		delta_t = 0
	quad.previous_time_takeoff = time.time()
	
	quad.target_alt_takeoff = target_alt

	# Check if altitude from px4flow is bad (i.e. alt = 0)
	quad.filtered_alt_takeoff = alt
	if alt == 0:
		quad.filtered_alt_takeoff = quad.previous_alt_takeoff
		if alt == 0 and quad.previous_alt_takeoff == 0:
			quad.filtered_alt_takeoff = quad.previous_filtered_alt_takeoff

	quad.previous_alt_takeoff = alt
	quad.previous_filtered_alt_takeoff = quad.filtered_alt_takeoff

	# Calculate P altitude error
	quad.error_alt_takeoff = target_alt - quad.filtered_alt_takeoff
	quad.previous_error_alt_takeoff = quad.error_alt_takeoff
			
	# Calculate I error
	quad.I_error_alt_takeoff = quad.I_error_alt_takeoff + quad.error_alt_takeoff*delta_t

	# Calculate RC integral gain
	RC_I = quad.I_error_alt_takeoff*quad.alt_K_I_takeoff

	# Calculate RC throttle value to send
	rc_T_send = quad.base_rc_throttle_takeoff + RC_I
	
	# Save controller pwm values
	quad.alt_RC_I_takeoff = RC_I
	T_send_takeoff = rc_filter(rc_T_send,quad.t_pwm_min,quad.t_pwm_max)
	quad.T_send_takeoff = round(T_send_takeoff)

	print quad.T_send_takeoff
	# Set RC throttle
	set_rc_throttle(master,quad,round(T_send_takeoff))
############################################################################################################
...
def alt_controller(master,quad,target_alt,alt):
	...
	# Have no derivative term when in dead zone of px4flow
	if quad.filtered_alt > 0.3:
...
def init_write_alt(filename):
	...
	f_alt.write("%Time, filtered_alt, RC_P, RC_I, RC_D, rc_T_send, x, y, z, phi, theta, psi, target_alt\n")
	f_alt.truncate()
	f_alt.close()
	return f_alt
############################################################################################################
...
############################################################################################################
def write_alt(filename,f_alt,alt,quad,vicon_pos,vicon_orient,flow_alt):
	f_alt = open(filename,'a+')
	f_alt.write("%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f\n" % (time.time(), quad.filtered_alt, quad.alt_RC_P, quad.alt_RC_I, quad.alt_RC_D, quad.T_send, vicon_pos[0], vicon_pos[1], vicon_pos[2], vicon_orient[0], vicon_orient[1], vicon_orient[2], quad.target_alt))
	f_alt.close()
	return f_alt
############################################################################################################
...
def init_write_flow(filename):
	...
	f_flow.write("%Time, target_alt, flow_alt, quality, x, y, z, phi, theta, psi\n")
	f_flow.truncate()
	f_flow.close()
	return f_flow
############################################################################################################
...
############################################################################################################
def write_flow(filename,f_flow,flow_alt,quality,quad,vicon_pos,vicon_orient):
	f_flow = open(filename,'a+')
	f_flow.write("%f, %f, %f, %f, %f, %f, %f, %f, %f, %f\n" % (time.time(), quad.target_alt, flow_alt, quality, vicon_pos[0], vicon_pos[1], vicon_pos[2], vicon_orient[0], vicon_orient[1], vicon_orient[2]))
	f_flow.close()
	return f_flow
############################################################################################################
...
############################################################################################################
def write_position(filename,f_pos,quad,vicon_pos,pix_att,flow_alt):
	f_pos = open(filename,'a+')
	f_pos.write("%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f\n" % (time.time(), quad.velx_inertial_send, quad.posx_P, quad.posx_I, quad.posx_D, quad.vely_inertial_send, quad.posy_P, quad.posy_I, quad.posy_D, vicon_pos[0], vicon_pos[1], flow_alt, pix_att[0], pix_att[1], pix_att[2], quad.target_x, quad.target_y))
	f_pos.close()
	return f_pos
############################################################################################################
...
############################################################################################################
def write_rc(filename,f_rc,quad,current_rc_channels,vicon_pos,pix_att,flow_alt):
	f_rc = open(filename,'a+')
	f_rc.write("%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f\n" % (time.time(), current_rc_channels[0], current_rc_channels[1], current_rc_channels[2], current_rc_channels[3], 0, 0, 0, 0, vicon_pos[0], vicon_pos[1], flow_alt, pix_att[0], pix_att[1], pix_att[2]))
	f_rc.close()
	return f_rc
############################################################################################################
...
class vehicle:
	def __init__(self):
		...
		self.base_rc_throttle_takeoff = 0
		self.base_rc_yaw = 0
		self.base_rc_roll = 0
		...
		self.filtered_alt = 0
		self.filtered_alt_takeoff = 0
		self.previous_filtered_alt = 0
		self.previous_filtered_alt_takeoff = 0
############################################################################################################
